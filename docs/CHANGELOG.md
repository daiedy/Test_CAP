...
### pipeline
- `npm run watch` переведён с `cds-serve --watch` (падает в cds 10: `this.load is not a function`) на `cds watch`; `npm start` на `cds serve`. Найдено агентом `test-ui`.
