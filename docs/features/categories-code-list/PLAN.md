...
| 9 | UI: аннотации Products | `fiori-app-dev` | `app/products/annotations/Products.cds` | Выбор значения из справочника (ADR-0011); Колонки таблицы, фильтры, шапка, секции | `mcp__fiori-mcp__search_docs` по `Common.Text`, `TextArrangement`, `ValueListWithFixedValues` до правки; `cds compile srv --to edmx-v4`: на `Products/category_code` ровно один `Common.ValueList` с `CollectionPath="Categories"`, `Common.ValueListWithFixedValues Bool="true"`, `Common.Text Path="category/name"`; grep по `category` в `app/` не находит строкового поля; выполнено 2026-09-07 (`fiori-app-dev`) |
| 10 | UI: аннотации Categories | `fiori-app-dev` | `app/products/annotations/Categories.cds` (новый), `app/products/annotations.cds` | Выбор значения из справочника (ADR-0011, `Common.Text` на ключе справочника) | `cds compile srv --to edmx-v4`: на `Categories/code` `Common.Text Path="name"` с вложенным `UI.TextArrangement TextOnly`; никаких `@UI.*` для `Categories`; выполнено 2026-09-07 (`fiori-app-dev`) |
