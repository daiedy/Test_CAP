...
- Versions in `.mcp.json` and in `npx` commands are pinned exactly; they are raised only by the `release-check` skill after reading the changelog.
- Hooks do not read or send out `~/.claude.json`, `.env`, service keys. Network calls only to the domains from `docs/framework/versions.json`.
