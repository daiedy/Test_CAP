...
- For a draft-enabled entity add `@Common.SemanticKey: [ <element> ]` naming the element that is both `UI.HeaderInfo.Title` and the first `@UI.LineItem` `DataField`: in OData V4 this is the only way the List Report row shows the draft or lock marker, because the `UI.HeaderInfo` Title/Description fallback for the editing status exists only in OData V2 (PATTERNS "Draft and lock marker in the List Report row", ADR-0015).
