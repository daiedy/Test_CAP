...
- Measured by fiori-app-dev: `fiori-mcp` 1.12.2 `list_functionality` has NO id for `filterFields.<key>` nor `liveMode` (filter bar: only `hideFilterBar`). User decision 2026-09-25: bounded ADR-0007 exception written as section "Exception to ADR-0007" inside ADR-0020 (scope = two named entries, hand edit only in a user-started `PIPELINE_ALLOW_PROTECTED=1` session, `run_manifest_validation` else ui5lint, diff check, exit = upstream-watcher re-checks each fiori-mcp bump). Any other manifest key (e.g. `label`) is outside it: stop and ask. Also note the PreToolUse hook blocks any Bash command whose text merely names `manifest.json` in a write-looking command; edit docs mentioning it with Edit, not python/sed.
