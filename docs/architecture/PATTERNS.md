...
| Draft and lock marker in the List Report row | `@Common.SemanticKey: [ <the element used as UI.HeaderInfo.Title> ]` in `app/<app>/annotations/<Entity>.cds`; the element must also be the first `@UI.LineItem` `DataField`; the responsive table renders the marker (`sap.m.ObjectMarker`) in that column and the cell becomes `sap.m.ObjectIdentifier`; the OData V2 `HeaderInfo` fallback does not exist in V4; no uniqueness constraint is implied. Every marker type is a real, interactive tab stop, including `Draft`; the icon-only-below-600px breakpoint documented for the standalone control does not necessarily reproduce inside the table cell (measure, do not assume) | `app/products/annotations/Products.cds` | ADR-0015 |
