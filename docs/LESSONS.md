...
- 2026-09-09, premise corrected 2026-09-10. Files written through `Bash` (redirection, heredoc, a script) instead of `Edit`/`Write` are invisible to PreToolUse `protect-files.mjs` and to PostToolUse `post-edit.mjs`, so for them the per-file-type `cds compile`, the Cyrillic and i18n scans, the protected-path denial (including `manifest.json`) and the ADR-0014 `edit` records never happen. The lint half is **not** blind: `subagent-stop.mjs` already lints whatever `changedFiles()` reports, whoever wrote it. Evidence from the marker run: `app/products/annotations/Products.cds` was changed by commit `1287c3c` and has zero `edit` records in `.pipeline/mcp-audit-*.jsonl`, so the MCP gate could not see the one file the feature actually changed. Four options were costed on 2026-09-10: (A) a PreToolUse `Bash` matcher that pattern-matches write tokens against protected paths - cheap, prevents rather than detects, but unbounded false negatives (`node -e`, a variable path, a generator); (B) a PostToolUse `Bash` hook re-running the per-file-type checks over `git status` - rejected, it cannot attribute a write to one agent among parallel ones and taxes the hottest tool; (C) extend the already git-based `subagent-stop.mjs` and Stop gates with the rest of the dispatch plus a protected-path audit - smallest diff, unevadable, but detects late; (D) documentation only - already tried and already failed, and it competes with a harness setting that nudges the model to edit through Bash. Recommendation: C, with A as a narrow guardrail. Status: `Pending user decision` - `.claude/**` and `scripts/hooks/**` are protected files.
