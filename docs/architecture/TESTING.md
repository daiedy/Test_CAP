...
5. Authorization: `defaults.auth = { username: 'alice' }` in every test file **that starts a server with `cds.test`** (including the contract test in `test/metadata.test.js`, because `@requires` on a service also protects `$metadata`) - a file that only spawns a script (e.g. `test/hooks-protect-bash.test.js`) needs no auth. Anonymous requests use `{ auth: null }` and assert `status` 401 (a **numeric** `code`); denied roles use `rejectedWith(/403/)` and assert `code: '403'` (a **string**); the access-denied test is a separate `it`. Mock users without a `password` accept any password, and `"*": true` authenticates an unknown user name without roles (403 on a `@restrict`ed entity, not 401).
...
- `cds compile ... --to edmx-v4` pretty-prints `$edmJson` expressions across several lines (e.g. `UI.CreateHidden` renders as `<Annotation Term="UI.CreateHidden">`, newline, `<Not>`, newline, `<Path>...</Path>`, newline, `</Not>`), so a contract assertion written as a single-line string never matches. Compare with the whitespace between tags removed (`data.replace(/>\s+</g, '><')`) and keep the term and its whole expression in one comparison, otherwise the test only proves the term exists somewhere (`test/metadata.test.js`, ADR-0013).
