...
1. Read the latest digest in `docs/framework/UPDATES.md` and `docs/architecture/STACK.md`. Make sure the working tree is clean, create the branch `chore/cds-$ARGUMENTS`.
...
5. Do not touch the MCP server versions; write a recommendation in `docs/framework/UPDATES.md` if a newer `@cap-js/mcp-server` has been released.
