...
Дата: 2026-09-07. Статус: утверждён (2026-09-07). Режим ворот: полуавтономный.
...
- [ ] List Report: фильтр «Category» это выпадающий список с множественным выбором из 6 названий, без диалога value help и без вкладки условий; после выбора «Kitchen» таблица содержит 3 товара, токен фильтра показывает название, не код; при выборе «Kitchen» и «Sports» 4 товара и два токена, запрос содержит `category_code eq 'KITCHEN' or category_code eq 'SPORTS'`.
- [ ] List Report: колонка «Category» показывает названия («Electronics»), не коды.
- [ ] Object Page: в шапке (Description) и в секции «General Information» показано название категории; в режиме редактирования поле «Category» это выпадающий список с одиночным выбором и маркером обязательности, элементы списка это названия; выбор другого значения сохраняется и отображается названием в поле и в шапке.
- [ ] Локаль `ru` (`?sap-ui-language=ru`): в фильтре, элементах списка, колонке и Object Page названия на русском («Электроника»), label «Категория».
- [ ] Элементы выпадающих списков (фильтр и форма) нигде не показывают коды ни в `en`, ни в `ru`.
- [ ] Клавиатурный сценарий из чеклиста доступности CONTEXT («Экраны», таблица доступности) проходит на обеих страницах; в дереве доступности поле объявлено как «Category»/«Категория», поле формы required, роль combobox.
- [ ] Консоль браузера без ошибок на List Report и Object Page.
- [ ] `npm run lint` в `app/products` (ui5lint) без ошибок, включая правило `prefer-test-starter` для новых тестовых страниц; `npm run start-mock` поднимается, List Report в мок-режиме показывает категории из `mockdata/Categories.json` (только `en`).
- [ ] OPA5-журнеи «filter products by category», «category is shown as a name», «edit category on the object page», «russian locale shows translated categories» проходят в `ui5-test-runner` против `npm run watch` + `npm start`; вывод раннера приложен к отчёту `test-ui` и к `VERIFICATION.md`.

Документация:

- [ ] `npm run docs:registry` выполнен, `node scripts/check-docs-fresh.mjs` зелёный; `docs/registry/DOMAIN-MODEL.md` содержит `my.catalog.Categories`, `SERVICES.md` показывает проекцию `Categories` как readonly и value help у `Products`, `UI-ARTIFACTS.md` показывает тесты `webapp/test/`.
- [ ] `docs/CHANGELOG.md`: строки по областям db, srv, app, test, deps, docs, включая явную запись об изменении контракта `category` → `category_code` и об установке `@sap-ux/ui5-test-writer`, `ui5-test-runner`.
- [ ] `docs/STATE.md`: активная фича закрыта, долги «мок-данные» и «тестов UI нет» закрыты, ADR-0010/0011 в накопленных решениях; `docs/architecture/STACK.md` и `TESTING.md` отражают установленные инструменты UI-тестов.
- [ ] `docs/features/categories-code-list/SUMMARY.md` и `VERIFICATION.md` заполнены; `PLAN.md` отмечен выполненным.
- [ ] `docs/architecture/PATTERNS.md`: примеры в строках «Справочник с выбором из списка» (`Categories`), «Переводимые тексты данных» (`Categories.name`), «Проверка существования цели ассоциации» (`Products.category`), «Выбор значения из справочника» (`Products.category_code`, формулировка по ADR-0011), «Сценарий пользователя» (`webapp/test/integration/`) обновлены; `templates/annotations-ui.cds` приведён к ADR-0011.
- [ ] `docs/decisions/ADR-0010`, `ADR-0011`: статус «принято» (сделано architect 2026-09-07); в ADR-0003 раздел «Последствия» пока не правится (указание оркестратора), отметка о выполнении делается в фазе документации по решению пользователя.
...
| 9 | UI: аннотации Products | `fiori-app-dev` | `app/products/annotations/Products.cds` | Выбор значения из справочника (ADR-0011); Колонки таблицы, фильтры, шапка, секции | `mcp__fiori-mcp__search_docs` по `Common.Text`, `TextArrangement`, `ValueListWithFixedValues` до правки; `cds compile srv --to edmx-v4`: на `Products/category_code` ровно один `Common.ValueList` с `CollectionPath="Categories"`, `Common.ValueListWithFixedValues Bool="true"`, `Common.Text Path="category/name"`; grep по `category` в `app/` не находит строкового поля |
| 10 | UI: аннотации Categories | `fiori-app-dev` | `app/products/annotations/Categories.cds` (новый), `app/products/annotations.cds` | Выбор значения из справочника (ADR-0011, `Common.Text` на ключе справочника) | `cds compile srv --to edmx-v4`: на `Categories/code` `Common.Text Path="name"` с вложенным `UI.TextArrangement TextOnly`; никаких `@UI.*` для `Categories` |
| 11 | UI: снимок и мок | `fiori-app-dev` | `app/products/webapp/localService/metadata.xml`, `localService/mockdata/Categories.json`, `Categories_texts.json`, `Products.json` | Обновление снимка metadata.xml; UI без бэкенда | `cds compile srv --to edmx-v4 -l en > app/products/webapp/localService/metadata.xml`; `npm run start-mock` поднимается, `/odata/v4/catalog/Categories` в моке отдаёт 6 записей, `Products` 15 |
| 12 | UI: тексты | `fiori-app-dev` | `app/products/webapp/i18n/*` | Тексты | по решению `ux-designer` новых ключей нет; шаг пропускается с пометкой в отчёте |
| 13 | Тесты UI | `test-ui` | `app/products/package.json` (devDependencies), `app/products/package-lock.json` (следствие установки), `app/products/webapp/test/testsuite.qunit.html`, `testsuite.qunit.js`, `webapp/test/integration/**` | Сценарий пользователя (OPA5 на `sap.fe.test.*`); правило `.claude/rules/tests-ui.md` | скиллы `ui5-best-practices-opa5`/`qunit` и `mcp__fiori-mcp__search_docs` («OPA5 Fiori elements», `JourneyRunner`) до правки; `npx ui5-test-runner --url http://localhost:8080/test/testsuite.qunit.html` при `npm run watch` (корень) и `npm start` (`app/products`), вывод в отчёт; `npm run lint` в `app/products` без ошибок |
