...
| Role-aware UI visibility | A read-only singleton `<Service>.Permissions` (`@odata.singleton @cds.persistence.skip @readonly`) served by an `on('READ')` handler in `srv/<name>-service.js` returning `req.user.is('<Role>')`; the UI reads it via `UI.CreateHidden`/`UI.UpdateHidden`/`UI.DeleteHidden` in `app/<app>/annotations/<Entity>.cds`, each `{ $edmJson: { $Not: { $Path: '/<Service>.EntityContainer/Permissions/<flag>' } } }`. A per-row element cannot answer this in a List Report toolbar (no row context, none at all on an empty list), so the singleton is the only mechanism | `srv/catalog-service.js`, `app/products/annotations/Products.cds` | ADR-0013 |
