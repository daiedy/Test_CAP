...
5. Manifest exception (after PLAN step 3, 2026-09-25): `fiori-mcp` 1.12.2 `list_functionality` offers no id for `filterFields.<key>` or `liveMode` (`research/fiori-mcp-custom-filter.md`), so, as a one-off exception to ADR-0007 recorded in ADR-0020 section "Exception to ADR-0007", `fiori-app-dev` adds exactly two `ProductsList` entries to `manifest.json` by hand: `controlConfiguration["@com.sap.vocabularies.UI.v1.SelectionFields"].filterFields.rating` (`template: products.ext.fragment.RatingRangeFilter`, `availability: Default`, `position { placement: After, anchor: price }`, no `label`, no `property`) and `liveMode: true`. Only in a session the user starts with `PIPELINE_ALLOW_PROTECTED=1`; validated with `run_manifest_validation`, or `ui5lint` alone on its known draft-06 error; the manifest diff holds nothing else. Every other or later manifest change goes through Fiori MCP again; `upstream-watcher` re-checks `list_functionality` on each `fiori-mcp` bump.
...
| List Report `ProductsList` (`app/products/webapp/manifest.json`) | `sap.fe.templates.ListReport`, `variantManagement: Page`, `initialLoad: true`, no `controlConfiguration`, Go button (no `liveMode`) | a custom filter field `rating` under `controlConfiguration` → `@com.sap.vocabularies.UI.v1.SelectionFields` → `filterFields`, placed after Price, shown by default, label taken from `Common.Label`; `liveMode: true` on `ProductsList` (no Go button); both by hand under the ADR-0020 exception (decision 5), nothing else in the manifest changes |
...
## Applicable patterns
- "Manifest change": Fiori MCP `list_functionality` → `get_functionality_details` → `execute_functionality`; the validation step uses `ui5lint` (`npm run lint` in `app/products`) because `run_manifest_validation` is broken (STATE open debt). Not applicable to this feature's two manifest entries: `fiori-mcp` 1.12.2 has no functionality for them, they are hand edits under the ADR-0020 exception (decision 5).
- "Custom section or column": fragment folder `ext/fragment/`.
- **Custom filter field: no row yet.** `docs/decisions/ADR-0020-custom-filter-field.md` (accepted 2026-09-25): one-way binding to `{filterValues>}` plus a `change` handler calling `setFilterValues`; section "Exception to ADR-0007" for the manifest.
