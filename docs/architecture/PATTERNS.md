...
| Authorization | `@requires: 'authenticated-user'` on the service, `@restrict` on the entity; mock users in `package.json` → `cds.requires.auth.users`. Entity grants cover the draft actions too (`draftEdit`, `draftPrepare`, `draftActivate`, discard need the same `grant`); UI tests authenticate via `app/<app>/ui5-test-runner.json` | `srv/catalog-service.cds` | ADR-0013 |
| Role-aware UI visibility | A read-only singleton `<Service>.Permissions` (`@odata.singleton @cds.persistence.skip @readonly`) served by an `on('READ')` handler in `srv/<name>-service.js` returning `req.user.is('<Role>')`; the UI reads it via `UI.CreateHidden`/`UI.UpdateHidden`/`UI.DeleteHidden` in `app/<app>/annotations/<Entity>.cds`, each `{ $edmJson: { $Not: { $Path: '/<Service>.EntityContainer/Permissions/<flag>' } } }`. A per-row element cannot answer this in a List Report toolbar (no row context, none at all on an empty list), so the singleton is the only mechanism. The mock fixture `mockdata/Permissions.json` is a one-element array like every mock file (an object form throws on the `$batch` path). Verify List Report and Object Page separately: an unresolved `$Not` binding defaulted to visible on one and hidden on the other (measured 2026-09-16) | `srv/catalog-service.js`, `app/products/annotations/Products.cds` | ADR-0013 |
...
| Table columns, filters, header, sections | `@UI.LineItem`, `@UI.SelectionFields`, `@UI.HeaderInfo`, `@UI.Facets` + `@UI.FieldGroup` in `app/<app>/annotations/<Entity>.cds`; a visual (non-text) column or field — a rating, a progress bar, a status indicator — is a `@UI.DataPoint #<Qualifier>` record (`Value`, `TargetValue`, `Visualization: #Rating`/`#Progress`/etc.) referenced by a `@UI.DataFieldForAnnotation` with `Target: '@UI.DataPoint#<Qualifier>'` and an explicit `Label` (the header does not fall back to `@title`) inside `UI.LineItem` and/or `UI.FieldGroup`; no `manifest.json` change, no fragment | `app/products/annotations/Products.cds` (`UI.DataPoint #Rating` → `sap.m.RatingIndicator` column and Object Page field, feature `products-rating-column`) | ADR-0004 |
...
| Custom filter field | `ext/fragment/<Name>.fragment.xml` + `ext/fragment/<Name>.js` (named pure functions, no controller extension); the control's value bound one-way from `filterValues>` with a `sap.fe.macros.filter.type.*` type and a `formatter`; its `change` handler calls `this.setFilterValues(<property>, <operator>, [values])` or clears the condition; wired via `controlConfiguration` → `@UI.SelectionFields` → `filterFields.<property>` (`template`, `availability`, `position`). `fiori-mcp` 1.12.2 has no functionality for `filterFields` or `liveMode`, so both are hand edits under the ADR-0020 exception to ADR-0007 until an upstream version covers them (STATE open debt) | `app/products/webapp/ext/fragment/RatingRangeFilter.*` (feature `products-rating-filter`) | ADR-0020 |
