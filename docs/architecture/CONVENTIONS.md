...
- Contract snapshot: `test/metadata.test.js` compares `cds compile srv --to edmx-v4` with the snapshot. Changed deliberately via `vitest -u` with a CHANGELOG entry.
