...
// Value help on the foreign key: show the text, hide the code (ADR-0011).
// ValueList is generated by the compiler from `sap.common.CodeList` (`@cds.odata.valuelist`);
// do not write `Common.ValueList` by hand here, it would suppress the generated one.
annotate OrdersService.Orders with {
  category @(
...
// The code list's own key needs Common.Text too, or dropdown/dialog rows show the code (ADR-0011).
annotate OrdersService.Categories with {
  code @(
    Common.Text            : name,
    Common.TextArrangement : #TextOnly
  );
