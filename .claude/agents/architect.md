...
- A change that alters what an endpoint answers to an unauthenticated caller (`@requires`, `@restrict`, a new auth kind) lists every caller outside the application in the plan and schedules their update in the same phase: the CI readiness probe in `.github/workflows/ci.yml`, the smoke curls of the `run-app` and `test-all` skills, the `ui-verifier` server check, README examples. A plan that asserted `ci.yml` unchanged left `main` red at the OPA5 job's `curl -sf $metadata` for two merges on 2026-09-23.
- A figure measured on a scratchpad or look-ahead copy (contract lines, EDMX diff, test counts) names the phase it describes. When phases split one change, the plan carries a figure per phase and each gate criterion uses its own; a single end-state number read as the phase-2 criterion cost a plan correction on 2026-09-11 (+50 for the feature, +20 for phase 2 alone).
