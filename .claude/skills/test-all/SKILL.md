...
7. Language check: `grep -rln "[А-Яа-яЁё]" db srv app/products/annotations app/products/webapp/ext app/products/webapp/test test scripts docs --exclude-dir=node_modules --exclude="*_ru.properties" --exclude="*.texts.csv" --exclude="*.snap" --exclude=ai-pipeline-plan.md` must return only test files that assert translated values.
