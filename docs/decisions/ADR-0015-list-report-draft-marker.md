...
- No project-owned user-facing string is added: `sap.m` ships `OM_DRAFT`, `OM_LOCKED_BY`, `OM_LOCKED_BY_ANOTHER_USER` and `OM_UNSAVED_BY` translated in `en` and `ru`, so invariant 7 is satisfied by adding nothing. The marker's appearance is the control's per-type default: `Draft` is text without an icon, `Locked*`/`Unsaved*` are icon plus text above a 600 px window width and icon only below it (`sap.m.ObjectMarker` 1.136.5). Nothing here is configurable from an annotation, and the accepted accessibility gap is the marker link's inline reactive area (WCAG 2.5.8), recorded as an upstream note rather than fixed with a custom column or CSS.
...
- [ ] `app/products/annotations/Products.cds`, `app/products/webapp/localService/metadata.xml`, `test/__snapshots__/metadata.test.js.snap`
- [ ] `test/metadata.test.js` (contract assertion), `app/products/webapp/test/integration/DraftMarkerInListReportJourney.js`, `opaTests.qunit.js`
- [ ] `docs/architecture/PATTERNS.md` (new row, two amended rows), `docs/STATE.md` (debt row removed), `docs/LESSONS.md`
