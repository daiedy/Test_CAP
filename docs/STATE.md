...
| По итогам ретро: `maxTurns` агентов `test-ui`, `ui-verifier`, `docs-keeper` малы (три остановки за прогон); скилл `feature` не обновляет STATE между фазами; нет smoke-проверки `npm run watch` в `test-all`; нет теста «metadata.xml актуален» | Правки `.claude/**` по решению пользователя (diff предложен в ретро); тест и smoke отдельной задачей | пользователь |
